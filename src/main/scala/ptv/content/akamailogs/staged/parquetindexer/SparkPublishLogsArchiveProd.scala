...
object SparkPublishLogsArchiveProd extends App {
  ...
  private def executeLogsArchivePublish(spark: SparkSession, eventdate: String, stagedDataPath: String) = {
    ...
    val staged = s"${stagedDataPath}dt=${eventdate}/"

    val sql_publish = s"INSERT INTO TABLE published_ott_dazn.massive_elb_logs PARTITION (dt=${eventdate}) " +
      s"SELECT substring(message,0,23) as app_timestamp, substring(message,25,4) as app_level, substring(message,30,6) as app_region, metadata ,logzio_id,beat,input_type,it,logzio_codec,message,offset,source,tags,token,type " +
      s"FROM massive_elb_logs_temp"
