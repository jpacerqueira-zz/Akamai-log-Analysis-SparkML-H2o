...
object SparkStageLogsArchiveProdWorkArround extends App{
  ...
  def executionRawStagedJob(spark: SparkSession, eventDate: String, rawDataPath: String, stagedDataPath: String,coalescefactor: Int) : Unit = {
    ...
    val df2=df1
      ...
      .dropNestedColumn("metadata.version")
      // Workarround after 20190226 13h30 UTC duplicate message.response in JSON log
      .withColumnRenamed("message.response","message.response1")
      .dropNestedColumn("message.response")
      .withColumnRenamed("message.response1","message.response")
      .coalesce(coalescefactor).persist(newLevel = StorageLevel.MEMORY_AND_DISK)
