...
echo "||____________________________________________________________________________________________________________________________________________________||"
echo "||_______________________  1.)  ______________________________________________________________________________________________________________________||"
echo "||____________________________________________________________________________________________________________________________________________________||"
echo "||                                                                                                                                                    ||"
echo "|| scala> val df1=sqlContext.read.parquet(\"/user/joao.cerqueira/data/staged/akamai/spark/dazn_logs\")                                                  ||"
echo "|| scala> df1.registerTempTable(\"dazn_logs\")                                                                                                          ||"
...
echo "|| scala> sqlContext.sql(\"select dt,hh,count(*) from dazn_logs where dt='2017-03-11' and hh between 0  and 24  group by dt,hh \").collect()            ||"
echo "||                                                                                                                                                    ||"
echo "||_______________________  2.)  ______________________________________________________________________________________________________________________||"
echo "||                                                                                                                                                    ||"
echo "|| scala> val df2=sqlContext.sql(\"select dt,hh,time,cs_uri_stem,count(*) as group_counter from dazn_logs \                                            ||"
echo "||   where dt='2017-03-11' and hh between 0  and 5  and cs_uri_stem LIKE '/out/u/%_video_%.mp4' AND   \                                               ||"
echo "||           sc_status=200 group by dt,hh,time,cs_uri_stem \")                                                                                         ||"
echo "|| df2: org.apache.spark.sql.DataFrame = [dt: string, hh: int, time: string, cs_uri_stem: string, group_counter: bigint]                              ||"
echo "|| scala>                                                                                                                                             ||"
echo "|| scala> df2.write.mode(\"append\").partitionBy(\"dt\",\"hh\").parquet(\"explore_data_dazn\")                                                                ||"
echo "||                                                                                                                                                    ||"
echo "||_______________________  3.)  ______________________________________________________________________________________________________________________||"
echo "||                                                                                                                                                    ||"
echo "|| scala>                                                                                                                                             ||"
echo "|| scala> val df1=sqlContext.read.parquet(\"/user/joao.cerqueira/data/staged/akamai/spark/dazn_logs\")                                                  ||"
echo "|| df1: org.apache.spark.sql.DataFrame = [date: string, time: string, cs_ip: string, cs_method: string, cs_uri_origin: string,                        ||"
...
echo "|| scala>  df1.registerTempTable(\"dazn_logs\")                                                                                                         ||"
echo "|| scala>exit()                                                                                                                                       ||"
echo "||_______________________  4.)  _________________________________________________________________________________________________________________________||"
echo "|| scala>                                                                                                                                             ||"
echo "|| scala>  val df1=sqlContext.read.parquet(\"/user/joao.cerqueira/data/staged/akamai/spark/dazn_logs\")                                                 ||"
echo "||  df1: org.apache.spark.sql.DataFrame = [date: string, time: string, cs_ip: string, cs_method: string, cs_uri_origin: string, cs_uri_stem: string,    ||"
echo "||  cs_uri_query: string, sc_status: int, sc_bytes: int, time_taken: int, cs_referer: string, cs_user_agent: string, cs_cookie: string,                 ||"
echo "||  cs_host_header: string, cs_request_id: string, cached: int, dt: string, hh: int]                                                                    ||"
echo "|| scala>                                                                                                                                             ||"
echo "||  scala> val df2=df1.withColumnRenamed(\"date\",\"date_v1\").withColumnRenamed(\"time\",\"time_v2\")                                                       ||"
echo "|| df2: org.apache.spark.sql.DataFrame = [date_v1: string, time_v2: string, cs_ip: string, cs_method: string, cs_uri_origin: string, cs_uri_stem: string, cs_uri_query: string, sc_status: int, sc_bytes: int, time_taken: int, cs_referer: string, cs_user_agent: string, cs_cookie: string, cs_host_header: string, cs_request_id: string, cached: int, dt: string, hh: int] i||"
echo "|| scala>  df2.coalesce(48).write.mode(\"append\").partitionBy(\"dt\",\"hh\").parquet(\"akamai_log_ottdazn\")                                                    ||" 
echo "|| scala>exit()                                                                                                                                       ||"
echo "||                                                                                                                                                    ||"
echo "||_______________________  5.)  _________________________________________________________________________________________________________________________||"
echo "||                                                                                                                                                       ||"
echo "|| scala>                                                                                                                                             ||"
echo "|| scala>  val df1=sqlContext.read.parquet(\"akamai_log_ottdazn\")                                                                                          ||"
echo "||  df1: org.apache.spark.sql.DataFrame = [date_v1: string, time_v2: string, cs_ip: string, cs_method: string, cs_uri_origin: string, cs_uri_stem: string,  ||"
echo "||  cs_uri_query: string, sc_status: int, sc_bytes: int, time_taken: int, cs_referer: string, cs_user_agent: string, cs_cookie: string, cs_host_header: string, ||"
echo "||  cs_request_id: string, cached: int, dt: string, hh: int]                                                                                             ||"
echo "|| scala>                                                                                                                                             ||"
echo "|| scala> df1.registerTempTable(\"dazn_logs\")                                                                                                             ||"
  
echo "|| scala>  sqlContext.sql(\"select dt,hh,count(*) from dazn_logs where dt='2017-03-11' and hh between 0  and 24  group by dt,hh \").collect()              ||"
echo "||  res1: Array[org.apache.spark.sql.Row] = Array([2017-03-11,0,2115194], [2017-03-11,1,2838873], [2017-03-11,2,2590211], [2017-03-11,3,2773231],        ||"
echo "||  [2017-03-11,4,9425564], [2017-03-11,5,33453108], [2017-03-11,6,50480075], [2017-03-11,7,76327865], [2017-03-11,8,44801010], [2017-03-11,9,14565259], ||"
echo "||  [2017-03-11,10,28145309], [2017-03-11,11,33250490], [2017-03-11,12,14968141], [2017-03-11,13,11170688], [2017-03-11,14,15038010],                    ||"
echo "||  [2017-03-11,15,19052908], [2017-03-11,16,12046173], [2017-03-11,17,3947788], [2017-03-11,18,3913426], [2017-03-11,19,2660343],                       ||"
echo "||  [2017-03-11,20,1503653], [2017-03-11,21,1582944], [2017-03-11,22,1719867], [2017-03-11,23,2160938])                                                  ||"
echo "|| scala> exit()                                                                                                                                         ||"
...
echo "||_______________________________________________________________________________________________________________________________________________________||"
cd ~/projects/akamai/execute_akamai/staged_akamai
echo "$SPARK_HOME"
echo "$SPARK_LOCAL_IP"
